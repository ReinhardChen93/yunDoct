<template>
  <div class="referralOut">
    ...
    <div class="headOuter" :style="{height:this.headHeight,backgroundColor:this.bgColor}">
      <commonHeader ref="head" :name="'转诊申请'" :color="color" :bgColor="bgColor" :isBack="isBack" @back="back" :isBlack="isBlack" :returnImg="returnImg"></commonHeader>
    </div>
    <!-- 内容 -->
    <div class="referralContent" :style="{marginTop:this.headHeight+'rem',height:this.contentHeight}">
      <!-- 协议头部 -->
      <p class="titleBox">
        基本信息（可扫描患者身份证获取）
      </p>
      ...
      <van-button type="info" @click="submitform" :disabled="isDisable">提交申请</van-button>
      <commonBottom/>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 获取患者信息,如果有memberId
    GetMember (memberId) {
      ajax.GetMember(memberId).then((res) => {
        let result = res.data.data
        if (res.data.succeed) {
          console.log(result)
          this.formData.MemName = result.MemName
          this.formData.MemIdcard = result.MemCard
          this.formData.MemSex = result.MemSex
          if (result.MemSex === 0) {
            this.sexValue = '男'
          } else {
            this.sexValue = '女'
          }
          this.formData.MemBirthday = result.BirthdayNew
          this.formData.MemTelphone = result.MemTel
          this.formData.MemAddress = result.Address
        } else {
          this.$toast(result.msg)
        }
      })
    },
    SaveRemoteReferra (formatas) {
      ajax.SaveRemoteReferra(formatas).then((res) => {
        let result = res.data.data
        console.log(res)
    ...
    submitform () {
      ...
      if (subStatus) {
        this.isDisable = true
        this.SaveRemoteReferra(this.formData)
      }
